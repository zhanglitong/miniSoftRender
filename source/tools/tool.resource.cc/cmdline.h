...
class LexicalCast_T<Target, Source, true>{
public:
  static Target cast(const Source &arg){
    return arg;
  }  
...
class LexicalCast_T<std::string, Source, false>{
public:
...
class LexicalCast_T<Target, std::string, false>{
public:
...
{
  return LexicalCast_T<Target, Source, detail::is_same<Target, Source>::value>::cast(arg);
}
...
template <class T>
struct RangeReader{
  RangeReader(const T &low, const T &high): low(low), high(high) {}
  T operator()(const std::string &s) const {
    T ret=DefaultReader<T>()(s);
    if (!(ret>=low && ret<=high)) throw CMDLine::CMDLineError("range_error");
    return ret;
  }
private:
  T low, high;
...
public:
  ...
  void add(const std::string &name,
    ...
    if (options.count(name)) throw CMDLineError("multiple definition: "+name);
  ...
  bool exist(const std::string &name) const {
    if (options.count(name)==0) throw CMDLineError("there is no flag: --"+name);
  ...
  const T &get(const std::string &name) const {
    ...
    if (p==NULL) throw CMDLineError("type mismatch flag '"+name+"'");
    return p->get();
  }
